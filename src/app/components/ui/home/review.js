...
export default function ReviewComponent() {
  ...
  const [isMuted, setIsMuted] = useState([true, true, true]); // Track mute state for each video
  ...
  const toggleMute = (index) => {
    ...
    const newMuteStates = [...isMuted];
    newMuteStates[index] = newMutedState;
    setIsMuted(newMuteStates);
  };
        ...
        <div className="mb-16 animate-fade-in-up">
                  ...
                  <div className="relative pt-[177.78%]"> {/* 9:16 aspect ratio for portrait */}
                    ...
                    <div className="absolute top-3 right-3 z-10">
                      <button
                        ...
                        className="bg-black/70 hover:bg-black/90 rounded-full p-2 transition-all duration-300 group/mute"
                        aria-label={isMuted[index] ? "Unmute video" : "Mute video"}
                      >
                        {isMuted[index] ? (
                          <svg className="w-5 h-5 text-white group-hover/mute:text-yellow-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="text-center mt-8">
            <p className="text-gray-500 text-sm flex items-center justify-center">
              ...
              Click on any video to play. Use the sound button to unmute for audio.
            </p>
          </div>
        </div>
